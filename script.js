...
const radioTrue = document.querySelector("#True");
const radioFalse = document.querySelector("#False");
...
function addBookToLibrary(e) {
  ...
  if (pagesbox.validity.rangeUnderflow) {
    document.querySelector(".error").innerText =
      "The minimum amout of pages is 5";
    e.preventDefault();
    return;
  }

  if (!radioFalse.checked && !radioTrue.checked) {
    document.querySelector(".error").innerText = "Specify whether it was read";
    e.preventDefault();
    return;
  }
...
function hideRadioValue() {
  ...
  for (let i in selection) {
    if (selection[i].checked) selection[i].checked = false;
  }
}
